...
这套东西我们叫它 **Harness**。它不是一个新的模型，也不是一个新的 IDE 插件，而是**一套完整的工程化工作流架构**。简单说，它解决的就是一句话——**怎么让 AI 不再"靠感觉"干活，而是像一支有纪律、有规范、有流程的研发团队那样工作**。
...
**首先，不是所有项目都适合上 Harness。** 咱们分三类：

✅ **强烈推荐**：Spring Boot / Java 后端项目（L4 有现成 ECC 技能）、中大型团队（≥ 3 人开发）、长期维护项目（≥ 6 个月）、多需求并发（≥ 2 个并发）。
...
**AI 自动补全**——这部分是 AI 提示词。**HyperSpec 上游的状态机详细文档这里不展开。**
